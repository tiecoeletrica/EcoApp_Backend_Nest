...
interface RewindStagingStageUseCaseRequest {
  stagingId: string;
  storekeeperId: string;
  comment?: string;
}

type RewindStagingStageResponse = Eihter<
  NotValidError | ResourceNotFoundError,
  null
>;

@Injectable()
export class RewindStagingStageUseCase {
  constructor(
  ...
  async execute({
    ...
    if (resultRewind.currentStage === resultRewind.nextStage)
      return left(
        new NotValidError(
          `Não é possível retroceder a separação '${staging.identifier}' que está no estágio '${staging.stage}'`
        )
      );
    ...
    const stagingTimestamp = StagingTimestamp.create({
      ...
      currentStage: resultRewind.currentStage,
      nextStage: resultRewind.nextStage,
      comment,
    });
